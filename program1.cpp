...
std::tuple<int, int, std::vector<int>> program1(int n, int W, std::vector<int> heights, std::vector<int> widths){
    ...
    for(int i = 0; i < n; i++){
        if(curr_width + widths.at(i) > W){ //if adding the painting would go beyond the width
            m += 1;
            cost += heights.at(i); //because the paitnings or decreasing in height, this will be the max for this platform
            curr_width = widths.at(i);
            platform_paintings.push_back(1);// create a new platform
        }
        else{
            if(i == 0){ //if this is the first painting being placed
